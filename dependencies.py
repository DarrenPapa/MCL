...
def frompip(module_name: str) -> bool:
    try:
        dist = importlib.metadata.distribution(module_name)
        return 'site-packages' in dist.locate()
    except (AttributeError, ValueError, importlib.metadata.PackageNotFoundError):
        return False
    ...
    while to_process:
        ...
        for imp in imports:
            if is_standard_library(imp):
                ...
                dependency_info[imp] = "  pip  "
            elif imp == "__main__":
                dependency_info[imp] = "  self "
            else:
                dependency_info[imp] = "unknown"
...
if __name__ == "__main__":
    script_name = "mcl.py"  # Replace with your script name
    dependencies = build_dependency_graph(script_name)
    
    print("Tree Output:")
    print_tree(script_name, dependencies)
